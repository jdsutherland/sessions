...
const INSTRUCTIONS =
  'Searchable history of every past AI coding session (Claude Code, Codex, Pi, omp, OpenCode, ds4-agent) on this machine — the conversations behind the commits. Decisions, rationale, abandoned approaches, and unfinished threads live here, not in git. ' +
  ...
  server.registerTool(
    'search_sessions',
    {
      ...
      inputSchema: {
        ...
        tool: z
          .enum(['claude', 'codex', 'pi', 'opencode', 'omp', 'ds4'])
          .optional()
          .describe('Filter to a specific tool'),
  ...
  server.registerTool(
    'get_context_primer',
    {
      ...
      inputSchema: {
        ...
        tool: z.enum(['claude', 'codex', 'pi', 'opencode', 'omp', 'ds4']).optional().describe('Filter to one tool.'),
        worktree: z
          .boolean()
          .optional()
...
function registerResources(server: McpServer): void {
  server.registerResource(
    ...
    }),
    {
      ...
      description: 'A past Claude Code, Codex, Pi, omp, OpenCode, or ds4-agent session transcript digest.',
