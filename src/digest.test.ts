...
/** Every transcript below is Claude-shaped, so the tool argument is noise in these calls. */
const digestOf = (lines: string[]): SessionDigest => buildSessionDigest(lines, 'claude');
...
describe('buildSessionDigest grouping', () => {
  test('pairs a user turn with its assistant reply', () => {
    const digest = digestOf(jsonl(user('fix the login bug'), assistant('done, patched auth.ts')));
    ...
    expect(digest.exchanges).toEqual([
      { index: 0, at: '', user: 'fix the login bug', assistant: 'done, patched auth.ts' },
    ]);
  });
  ...
  test('assistant messages before the first genuine turn belong to no exchange', () => {
    const digest = digestOf(jsonl(assistant('session preamble'), user('real question'), assistant('answer')));
    expect(digest.exchangeCount).toBe(1);
  ...
  test('session ending mid-exchange yields an empty assistant', () => {
    const digest = digestOf(jsonl(user('done?'), assistant('yes'), user('one more thing')));
    expect(digest.exchangeCount).toBe(2);
    expect(digest.exchanges[1]).toEqual({ index: 2, at: '', user: 'one more thing', assistant: '' });
  });

  test('exchange indices are valid message offsets', () => {
    const lines = jsonl(
...
describe('buildSessionDigest degenerate sessions', () => {
  test('zero genuine turns → empty exchanges, no throw', () => {
    const digest = digestOf(jsonl(injected('hook one'), injected('hook two'), assistant('unowned')));
...
describe('buildSessionDigest truncation and budget', () => {
  test('giant single message is clipped to field max, not blowing the budget', () => {
    const digest = digestOf(jsonl(user('x'.repeat(10_000)), assistant('y '.repeat(5_000))));
  ...
  test('truncation cuts at a word boundary with an ellipsis', () => {
    ...
    const digest = digestOf(jsonl(user(words), assistant(words)));
  ...
  test('elided + exchanges.length always equals exchangeCount', () => {
    const digest = digestOf(sessionOf(600));
...
describe('renderDigestMarkdown', () => {
  test('renders one block per exchange with the message index', () => {
    const digest = digestOf(jsonl(user('do it'), assistant('did it')));
  ...
  test('places an elision marker between head and tail', () => {
    ...
    const digest = digestOf(jsonl(...rows));
  ...
  test('explains itself for sessions with no genuine turns', () => {
    const digest = digestOf(jsonl(injected('hook only')));
